...
suite "Block Advertising and Discovery":
  ...
  setup:
    ...
    switch = SwitchBuilder.new().withNoise().withMplex(5.minutes, 5.minutes).withTcpTransport({ServerFlags.ReuseAddr}).withAddresses(@[MultiAddress.init("/ip4/127.0.0.1/tcp/0").tryGet()]).build()
...
suite "E2E - Multiple Nodes Discovery":
  ...
  setup:
    ...
    for _ in 0 ..< 4:
      ...
      let
        s = SwitchBuilder.new().withNoise().withMplex(5.minutes, 5.minutes).withTcpTransport({ServerFlags.ReuseAddr}).withAddresses(@[MultiAddress.init("/ip4/127.0.0.1/tcp/0").tryGet()]).build()
        blockDiscovery = MockDiscovery.new()
