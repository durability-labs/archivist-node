...
from "../../../archivist/node" import retrieve, fetchManifest
from libp2p import Cid, init, `$`

logScope:
  topics = "libarchivist libarchivistdownload"
...
proc cleanupRequest(self: ptr NodeDownloadRequest) =
  if not self.isNil:
    deallocShared(self[].cid)
    deallocShared(self[].filepath)

proc init(
    archivist: ptr NodeServer, cCid: cstring = "", chunkSize: csize_t = 0, local: bool
): Future[Result[string, string]] {.async: (raises: []).} =
  let cid = Cid.init($cCid)
  if cid.isErr:
    return err("Failed to download: cannot parse cid: " & $cCid)
  ...
  try:
    let res = await archivist[].archivistNode.retrieve(cid.get(), local)
    if res.isErr():
...
proc chunk(
    archivist: ptr NodeServer, cCid: cstring = "", onChunk: OnChunkHandler
): Future[Result[string, string]] {.async: (raises: []).} =
  let cid = Cid.init($cCid)
  if cid.isErr:
    return err("Failed to download chunk: cannot parse cid: " & $cCid)
...
proc stream(
  ...
  try:
    let res =
      await noCancel archivist.streamData(session.stream, onChunk, chunkSize, filepath)
    if res.isErr:
...
proc manifest(
  ...
  try:
    let manifestRes = await archivist[].archivistNode.fetchManifest(cid.get())
    if manifestRes.isErr:
      return err("Failed to fetch manifest: " & manifestRes.error.msg)

    return ok(serde.toJson(manifestRes.get()))
  except CancelledError:
    return err("Failed to fetch manifest: download cancelled.")

proc process*(
    self: ptr NodeDownloadRequest, archivist: ptr NodeServer, onChunk: OnChunkHandler
): Future[Result[string, string]] {.async: (raises: []).} =
  defer:
    destroyShared(self)
  ...
  of NodeDownloadMsgType.INIT:
    let res = (await init(archivist, self.cid, self.chunkSize, self.local))
    if res.isErr:
      error "Failed to INIT.", error = res.error
      return err($res.error)
    return res
  of NodeDownloadMsgType.CHUNK:
    let res = (await chunk(archivist, self.cid, onChunk))
    if res.isErr:
  ...
  of NodeDownloadMsgType.STREAM:
    let res = (
      await stream(
        archivist, self.cid, self.chunkSize, self.local, self.filepath, onChunk
      )
    )
    if res.isErr:
  ...
  of NodeDownloadMsgType.CANCEL:
    let res = (await cancel(archivist, self.cid))
    if res.isErr:
      error "Failed to CANCEL.", error = res.error
      return err($res.error)
    return res
  of NodeDownloadMsgType.MANIFEST:
    let res = (await manifest(archivist, self.cid))
    if res.isErr:
